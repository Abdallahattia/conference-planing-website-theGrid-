...
import {addField, allFulfilled, noop} from '../utils/utils.js';
...
export default function wixFinish(state, interactive) {
    ...
    return allFulfilled([getCDN(window), model, interactive, dcl(window)]).then(([cdn, {visitorId}]) => {
        ...
        const bsi = readCookie(window, 'bSession');
        const result = {
            entryType,
            isSsr,
...
function getCDN({URL}) {
    ...
    const {origin} = new URL(config.src);
